...
public class GroupConfigTest {
  ...
  public void testTransfer() throws Exception {
    ...
    dataSource.setDsdefItem(
        new DataSourceEntity.Reference(
            "device_pineapple", "/resource_1/23", "kumquat", BaseUtil.getNowAsEpochMillis(), 0));
    ...
    assertThat(config.getDss().get(0))
        .hasFieldOrPropertyWithValue("classInfo", dataSource.getClassInfo())
        .hasFieldOrPropertyWithValue("description", dataSource.getDescription())
        .hasFieldOrPropertyWithValue("n", dataSource.getDsn())
        .hasFieldOrPropertyWithValue("t", dataSource.getDst())
        .hasFieldOrPropertyWithValue("threshLow", dataSource.getThreshLow())
        .hasFieldOrPropertyWithValue("threshHigh", dataSource.getThreshHigh())
        .hasFieldOrProperty("operate");
    assertThat(config.getDss().get(0).getOperate())
        .isNotNull()
    ...
    assertThat(config.getDss().get(0).getAttributeList())
        .extracting("an")
        .containsOnly("lemon", "grape");
    assertThat(config.getDss().get(0).getD())
        .hasFieldOrPropertyWithValue("d", dataSource.getLatestReference().getDsri().getDi())
        .hasFieldOrPropertyWithValue("url", dataSource.getLatestReference().getDsri().getResUri())
        .hasFieldOrPropertyWithValue("pn", dataSource.getLatestReference().getDsri().getPt())
        .hasFieldOrPropertyWithValue("s", dataSource.getLatestReference().getDsrf());
