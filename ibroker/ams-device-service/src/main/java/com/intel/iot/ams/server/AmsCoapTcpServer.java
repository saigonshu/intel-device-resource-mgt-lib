...
public class AmsCoapTcpServer extends CoapServer {

  public AmsCoapTcpServer(boolean logOn) {
  ...
  /**
   * Add individual endpoints listening on default CoAP port on all IPv4 addresses of all network
   * interfaces.
   */
  private void addEndpoints(boolean logOn) {
    NetworkConfig netCfg =
        new NetworkConfig().setInt(NetworkConfig.Keys.MAX_MESSAGE_SIZE, 1024 * 1024 * 100)
                           .setInt(NetworkConfig.Keys.MAX_RESOURCE_BODY_SIZE, 1024 * 2)
                           .setInt(NetworkConfig.Keys.PREFERRED_BLOCK_SIZE, 1024 * 2)
    ...
    Connector serverConnector = new TcpServerConnector(bindToAddress, 1, 100000);
    CoapEndpoint endpoint = new CoapEndpoint(serverConnector, netCfg);
    if (logOn) {
