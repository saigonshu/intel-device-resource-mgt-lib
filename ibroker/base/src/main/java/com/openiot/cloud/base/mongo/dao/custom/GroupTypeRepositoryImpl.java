...
public class GroupTypeRepositoryImpl implements GroupTypeRepositoryCustom {
  ...
  public List<GroupType> filter(
              ...
              operations.add(
                  Aggregation.match(
                      "null".equals(prj)
                          ...
                          : new Criteria()
                              .orOperator(
                                  where(ConstDef.F_PROJECT).is(prj),
                                  where(ConstDef.F_PROJECT).is(null)))));
    }
    ...
    displayName.ifPresent(
        dpn -> operations.add(Aggregation.match(where(ConstDef.F_DISPLAYNAME).is(dpn))));
    ...
    if (!withCfg.isEmpty()) {
      mpara.add(
          where(String.format("%s.%s", ConstDef.F_CONFIGS, ConstDef.F_CONFIGNAME)).in(withCfg));
    }
    // $match : {"dss.dsn" : { $in : dss}}
    if (!withDs.isEmpty()) {
      mpara.add(
          where(String.format("%s.%s", ConstDef.F_DATASOURCES, ConstDef.F_DATASOURCENAME))
              .in(withDs));
    }
            ...
            if (!BaseUtil.isNonequivalentQuery(k, v)) {
              ...
              mpara.add(
                  where(ConstDef.F_ATTRS)
                      .elemMatch(where(ConstDef.F_ATTRNAME).is(k).and(ConstDef.F_ATTRVALUE).is(v)));
            } else {
              mpara.add(
                  getNonequivalentQueryForAttrAndCfg(
                      ConstDef.F_ATTRS, ConstDef.F_ATTRNAME, ConstDef.F_ATTRVALUE, k, v));
            }
          });
    }
            ...
            if (!BaseUtil.isNonequivalentQuery(k, v)) {
              ...
              mpara.add(
                  where(ConstDef.F_CONFIGS)
                      .elemMatch(
                          where(ConstDef.F_CONFIGNAME).is(k).and(ConstDef.F_CONFIGVALUE).is(v)));
            } else {
              mpara.add(
                  getNonequivalentQueryForAttrAndCfg(
                      ConstDef.F_CONFIGS, ConstDef.F_CONFIGNAME, ConstDef.F_CONFIGVALUE, k, v));
            }
          });
    }
  ...
  private Criteria getNonequivalentQueryForAttrAndCfg(
      String item, String keyInItem, String valueInItem, String queryKey, String queryValue) {
    ...
    return where(item)
        .elemMatch(
            new Criteria()
                .andOperator(
                    where(keyInItem).is(queryKey), query.apply(valueInItem).apply(queryValue)));
  }
  ...
  public GroupType createOrUpdateOne(GroupType newGroupType) {
    ...
    Optional<Map<String, String>> valResult =
        groupType
            .map(gt -> BaseUtil.validate(validator, gt, CreateValidator.class))
