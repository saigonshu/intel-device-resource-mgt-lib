...
public class DevSessRepositoryImpl implements DevSessRepositoryCustom {
  ...
  public List<DevSession> filter(
    ...
    q.addCriteria(
        new Criteria()
            .orOperator(
                where(ConstDef.F_BEGEIN).gte(from).andOperator(where(ConstDef.F_BEGEIN).lte(to)),
                where(ConstDef.F_END).gte(from).andOperator(where(ConstDef.F_END).lte(to))));
