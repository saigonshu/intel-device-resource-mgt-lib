...
public class GroupRepositoryImpl implements GroupRepositoryCustom {
  ...
  @Autowired GroupTypeRepository gtRepo;

  @Autowired MongoTemplate mtemplate;
          ...
          Optional.ofNullable(g.getGtdtl())
              ...
              .ifPresent(
                  gtDss -> {
                    ...
                    if (dataSourceListGroup == null) return;
                    gtDss.stream()
                        .filter(
                            dataSourceGroupType ->
                                !dataSourceListGroup.contains(dataSourceGroupType))
  ...
  public List<Group> filter(
    ...
    projectId.ifPresent(
        proj -> operations.add(Aggregation.match(where(ConstDef.F_PROJECT).is(proj))));
    ...
    // $match : {"gt" : {$in : gtNames}}
    gtNames
        .filter(gtns -> !gtns.isEmpty())
        .ifPresent(gtns -> operations.add(Aggregation.match(where(ConstDef.F_GRPTYPE).in(gtns))));

    // $match : {"dpn" : {$in : dpNames}}
    dpNames
        .filter(dpns -> !dpns.isEmpty())
        .ifPresent(
            dpns -> operations.add(Aggregation.match(where(ConstDef.F_DISPLAYNAME).in(dpns))));

    // $lookup : {from : "GroupType", localField : "gt", foreignField :
    // "_id", as : "gtdtl"
    operations.add(
        Aggregation.lookup(
            ConstDef.C_GRPTYPE, ConstDef.F_GRPTYPE, ConstDef.F_ID, ConstDef.F_GTDETAIL));
    ...
    // dsintids}}]}
    dsInternalIds
        .filter(dsintids -> !dsintids.isEmpty())
        .ifPresent(
            dsintids ->
                mpara.add(
                    new Criteria()
                        .orOperator(
                            where(
                                    String.format(
                                        "%s.%s",
                                        ConstDef.F_DATASOURCES, ConstDef.F_DATASOURCEINTLID))
                                .in(dsintids),
                            where(
                                    String.format(
                                        "%s.%s.%s",
            ...
            map -> {
              map.forEach(
                  (k, v) -> {
                    mpara.add(
                        where(ConstDef.F_MR)
                            .elemMatch(where(ConstDef.F_DEVID).is(k).and(ConstDef.F_RESURI).is(v)));
                  });
            });
        ...
        .ifPresent(
            attrs ->
                mpara.add(
                    new Criteria()
                        .orOperator(
                            where(String.format("%s.%s", ConstDef.F_ATTRS, ConstDef.F_ATTRNAME))
                                .in(attrs),
                            where(
                                    String.format(
                                        "%s.%s.%s",
                                        ConstDef.F_GTDETAIL, ConstDef.F_ATTRS, ConstDef.F_ATTRNAME))
                                .in(attrs))));
        ...
        .ifPresent(
            cfgs ->
                mpara.add(
                    new Criteria()
                        .orOperator(
                            where(String.format("%s.%s", ConstDef.F_CONFIGS, ConstDef.F_CONFIGNAME))
                                .in(cfgs),
                            where(
                                    String.format(
                                        "%s.%s.%s",
                ...
                mpara.add(
                    new Criteria()
                        .orOperator(
                            where(
                                    String.format(
                                        "%s.%s", ConstDef.F_DATASOURCES, ConstDef.F_DATASOURCENAME))
                                .in(dss),
                            where(
                                    String.format(
                                        "%s.%s.%s",
                  ...
                  (k, v) -> {
                    if (!BaseUtil.isNonequivalentQuery(k, v)) {
                      mpara.add(
                          new Criteria()
                              .orOperator(
                                  where(ConstDef.F_ATTRS)
                                      .elemMatch(
                                          where(ConstDef.F_ATTRNAME)
                                              .is(k)
                                              .and(ConstDef.F_ATTRVALUE)
                                              .is(v)),
                                  where(
                                          String.format(
                                              "%s.%s", ConstDef.F_GTDETAIL, ConstDef.F_ATTRS))
                                      .elemMatch(
                                          where(ConstDef.F_ATTRNAME)
                                              .is(k)
                                              .and(ConstDef.F_ATTRVALUE)
                      ...
                      mpara.add(
                          getNonequivalentQueryForAttrAndCfg(
                              ConstDef.F_ATTRS,
                              String.format("%s.%s", ConstDef.F_GTDETAIL, ConstDef.F_ATTRS),
                              ConstDef.F_ATTRNAME,
                  ...
                  (k, v) -> {
                    if (!BaseUtil.isNonequivalentQuery(k, v)) {
                      mpara.add(
                          new Criteria()
                              .orOperator(
                                  where(ConstDef.F_CONFIGS)
                                      .elemMatch(
                                          where(ConstDef.F_CONFIGNAME)
                                              .is(k)
                                              .and(ConstDef.F_CONFIGVALUE)
                                              .is(v)),
                                  where(
                                          String.format(
                                              "%s.%s", ConstDef.F_GTDETAIL, ConstDef.F_CONFIGS))
                                      .elemMatch(
                                          where(ConstDef.F_CONFIGNAME)
                                              .is(k)
                                              .and(ConstDef.F_CONFIGVALUE)
                      ...
                      mpara.add(
                          getNonequivalentQueryForAttrAndCfg(
                              ConstDef.F_CONFIGS,
                              String.format("%s.%s", ConstDef.F_GTDETAIL, ConstDef.F_CONFIGS),
                              ConstDef.F_CONFIGNAME,
    ...
    if (!mpara.isEmpty()) {
      operations.add(
          Aggregation.match(new Criteria().andOperator(mpara.toArray(new Criteria[mpara.size()]))));
    }
  ...
  public List<Group> findAllGroupByRes(String di, String uri) {
    ...
    Query query =
        Query.query(
            where(ConstDef.F_MR)
                .elemMatch(where(ConstDef.F_DEVID).is(di).and(ConstDef.F_RESURI).is(uri)));
    List<Group> groupList = mtemplate.find(query, Group.class, ConstDef.C_GRP);
      ...
      if (fromGroup != null) {
        ...
        if (group.getGt() != null) {
          List<GroupType> groupTypeList =
              mtemplate.find(
                  Query.query(where(ConstDef.F_ID).is(group.getGt())),
                  GroupType.class,
                  ConstDef.C_GRPTYPE);
  ...
  public Group createOrUpdateOne(Group newGroup) {
    ...
    if (existingGroup.isPresent()) {
      valResult =
          group
              .map(g -> BaseUtil.validate(validator, g, UpdateValidator.class))
              .filter(result -> !result.isEmpty());
    } else {
      valResult =
          group
              .map(g -> BaseUtil.validate(validator, g, CreateValidator.class))
              .filter(result -> !result.isEmpty());
    }
  ...
  public DataSourceEntity findOneDsByInternalId(String internalId) {
    ...
    Query query =
        Query.query(
            where(String.format("%s.%s", ConstDef.F_DATASOURCES, ConstDef.F_DATASOURCEINTLID))
                .is(internalId));
    List<Group> groupList = mtemplate.find(query, Group.class, ConstDef.C_GRP);
    if (groupList.isEmpty()) {
      return null;
  ...
  private Criteria getNonequivalentQueryForAttrAndCfg(
      String itemInGrp,
      String itemInGrpTyp,
      String keyInItem,
      String valueInItem,
        ...
        .orOperator(
            where(itemInGrp)
                .elemMatch(
                    new Criteria()
                        .andOperator(
                            where(keyInItem).is(queryKey),
                            query.apply(valueInItem).apply(queryValue))),
            where(itemInGrpTyp)
                .elemMatch(
                    new Criteria()
                        .andOperator(
  ...
  private Group assembleGroup(Group group) {
    ...
    if (dataSourceListGroupType != null) {
      dataSourceListGroupType.stream()
          .filter(
              dataSourceTempalte ->
                  dataSourceListGroup == null || !dataSourceListGroup.contains(dataSourceTempalte))
  ...
  public static void mergeDss(DataSourceEntity dataSource, DataSourceEntity dataSourceTemplate) {
    ...
    if (attributeListDataSourceTemplate != null) {
      attributeListDataSourceTemplate.stream()
          .filter(
              attributeDataSourceTemplate ->
                  attributeListDataSource == null
    ...
    if (dataSource.getInterval() == null) dataSource.setInterval(dataSourceTemplate.getInterval());
  }
}
