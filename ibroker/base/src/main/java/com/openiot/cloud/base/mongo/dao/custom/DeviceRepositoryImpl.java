...
public class DeviceRepositoryImpl implements DeviceRepositoryCustom {
  ...
  public List<Device> filter(
    ...
    attribute.ifPresent(
        attr ->
            q.addCriteria(
                where(String.format("%s.%s", ConstDef.F_CONFIGS, ConstDef.F_ATTRS))
                    .elemMatch(
                        Criteria.where(ConstDef.F_ATTRNAME)
                            .is(attr.getAn())
