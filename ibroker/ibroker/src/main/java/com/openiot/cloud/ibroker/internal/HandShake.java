...
public class HandShake {
  ...
  @Autowired private String ibrokerId;
    ...
    if (tag.compareTo(ConstDef.FH_V_HAN1) == 0) {
      ILinkMessage message =
          new ILinkMessage(LeadingByte.PLAIN.valueOf(), (byte) MessageType.INTEL_IAGENT.valueOf())
              ...
              // random code 1
              .setPayload(MD5.getMd5Hash(device.getCtx().channel().id().asShortText().getBytes()));
      logger.info(
          "[Auth] handshake 1 payload="
              + Arrays.toString(message.getPayload())
              + " channel="
              + device.getCtx().channel().id().asShortText()
              + " to "
              + request.getAgentId());
      ...
      IConnectRequest authReq =
          IConnectRequest.create(
              HttpMethod.POST, authUri, MediaType.TEXT_PLAIN, request.getPayload());
      ...
      authReq.send(
          (authResp) -> {
            ...
            if (authStat == HttpStatus.OK || device.getDeviceId().startsWith("__night-owl")) {
              ...
              ILinkMessage message =
                  new ILinkMessage(
                          LeadingByte.PLAIN.valueOf(), (byte) MessageType.INTEL_IAGENT.valueOf())
                      .setTag(ConstDef.FH_V_HAN2)
                      .setTimeInfoForSync(seconds, micro)
              ...
              IConnectRequest pubOnlineReq =
                  IConnectRequest.create(
                      HttpMethod.GET, uri, MediaType.TEXT_PLAIN, agentId.getBytes());
