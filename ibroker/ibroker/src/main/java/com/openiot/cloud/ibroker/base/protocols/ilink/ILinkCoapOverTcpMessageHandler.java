...
public class ILinkCoapOverTcpMessageHandler {
  ...
  @Autowired private ResourceManager resourcesForCoapOverTcp;
  @Autowired private DefaultJmsHandler jmsHandler;
  @Autowired public static final byte[] COAP_MESSAGE_ETAG = new byte[] {0x70, 0x07, 0x70, 0x07};
  ...
  public void onMessage(Device srcDevice, ILinkMessage request) {
    ...
    if (srcDevice.getDeviceId() == null) {
      logger.warn(
          String.format(
              "device %s @ channel %s doesn't have the device id",
              srcDevice, ((IAgent) srcDevice).getRequestChannel()));
      return;
    }
    ...
    CompletableFuture.supplyAsync(() -> decodeAsCoapMessage(request.getPayload()))
        .thenApplyAsync(
            coapMessage ->
                Optional.ofNullable(coapMessage)
                    .map(sameCoapMessage -> processCoapMessage(request, sameCoapMessage))
                    .orElse(null))
        .thenAcceptAsync(
            coapMessage ->
                Optional.ofNullable(coapMessage)
                    .ifPresent(
                        sameCoapMessage ->
                            dispatchCoapRequest(request, srcDevice, sameCoapMessage)));
  }
  ...
  private void dispatchCoapRequest(ILinkMessage request, Device srcDevice, CoapMessage cm) {
    ...
    logger.debug(
        String.format(
            "dispatch %s to the %s", cm, dispatchToJms ? "message queue" : "COAP+TCP service"));
    if (!dispatchToJms) {
  ...
  public static byte[] frameToken(byte[] ilinkMessageId, byte[] origToken) {
    if (ilinkMessageId.length != 4 || origToken.length == 0) {
      logger.warn(
          String.format(
              "an invalide ilink message id(%s) or token(%s)",
              ilinkMessageId.length != 4, origToken.length == 0));
      return null;
    }
    ...
    logger.debug(
        String.format(
            "--> [token] aft frame %s, %s -> %s",
            Arrays.toString(ilinkMessageId), Arrays.toString(origToken), Arrays.toString(token)));

    return token;
  }

  public static ILinkMessage restoreMessageIDAndToken(
      byte[] token, CoapMessage cm, ILinkMessage ilink) {
    ...
    logger.debug(
        String.format(
            "--> [token] aft restore %s -> %s, %s",
            Arrays.toString(token), Arrays.toString(ilinkMessageId), Arrays.toString(origToken)));
  ...
  public static CoapMessage decodeAsCoapMessage(byte[] payload) {
    ...
    try {
      ...
      Method decode =
          ReflectionUtils.findMethod(
              CoapDecoder.class, "decode", ChannelHandlerContext.class, ByteBuf.class, List.class);
