...
public class ProvisionProjectAmqpHandler implements IConnectServiceHandler {
  ...
  void queryProject(IConnectRequest request) throws IOException {
    ...
    if (queryParams.containsKey(ConstDef.Q_DEVID)
      ...
      gatewayList =
          Stream.of(
                  gatewayService.findByIAgentIdAndProjectId(
                      queryParams.get(ConstDef.Q_DEVID), queryParams.get(ConstDef.Q_PROJECTID)))
              .collect(Collectors.toList());
    } else if (queryParams.containsKey(ConstDef.Q_DEVID)) {
      // di only
      gatewayList =
          Stream.of(gatewayService.findByIAgentId(queryParams.get(ConstDef.Q_DEVID)))
              .collect(Collectors.toList());
    } else {
      // pi only
    ...
    if (projectDTO == null) {
      IConnectResponse.createFromRequest(
              ...
              objectMapper.writeValueAsBytes(
                  new ErrorMessage(
                      String.format(
                          "%s is not an existed project", queryParams.get(ConstDef.Q_PROJECTID)))))
          .send();
      return;
    }
