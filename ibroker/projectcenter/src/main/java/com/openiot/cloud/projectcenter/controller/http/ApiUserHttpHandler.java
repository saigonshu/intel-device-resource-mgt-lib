...
public class ApiUserHttpHandler {
  ...
  public ResponseEntity<?> updateUser(
      @RequestParam(ConstDef.Q_NAME) String uesrName, @RequestBody UserAO userAO) {
    TokenContent tokenContent =
        Optional.ofNullable(SecurityContextHolder.getContext())
            .map(SecurityContext::getAuthentication)
            .filter(authentication -> authentication.getDetails() instanceof TokenContent)
  ...
  public ResponseEntity<?> queryUser(
      @RequestParam(value = ConstDef.Q_NAME, required = false) String userName,
    ...
    List<UserAO> userAOS =
        userService.queryUser(userName, projectId, tokenContent).stream()
            .map(
                userDTO -> {
